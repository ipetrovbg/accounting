...
export function transactionManageReducer(state: TransactionManageState = initialState.transactionManage, action: TransactionMangeActions): TransactionManageState {
    switch (action.type) {
        ...
        const { id, date, isTest, createdAt, updatedAt, reason, userId, withdrawal, deposit } = action.transaction;

            return { ...state, ...{ id, date, isTest, createdAt, updatedAt, reason, userId, withdrawal, deposit } };
